 ...
 */
class Luclong_Tools_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function appendAttributeAction(){
        ...
        if(count($params) > 0){
            foreach ($params as $param){
                ...
                $varPath =  Mage::getBaseDir("var").'/tools/'.$param.".txt";
                //$attribute = Mage::getModel('eav/config')->getAttribute('catalog_product', $param
                $attribute = Mage::getModel('eav/config')->getAttribute('catalog_product', $param);
                ...
                foreach ($attribute->getSource()->getAllOptions(true, true) as $option){
                     $attribute_exist_option[] = $option['label'];
                }
                ...
                for($i = 0; $i < count($option_label_add); $i++ ){
                    $this->addAttributeValue($param, $option_label_add[$i]);
                }
            }
        }
    }
