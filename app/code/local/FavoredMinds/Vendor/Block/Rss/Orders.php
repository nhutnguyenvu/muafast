...
class FavoredMinds_Vendor_Block_Rss_Orders extends Mage_Core_Block_Template {
   ...
   * @var string
   */
  const CACHE_TAG = 'block_html_vendor_rss_new_orders';
  ...
  protected function _construct() {
    ...
    */
    $this->setCacheKey('vendor_rss_orders');
  ...
  protected function _toHtml() {
    ...
    $collection = $order->getCollection()
            ->addAttributeToFilter('main_table.created_at', array('date'=>true, 'from'=> $passDate))
    ...
    $detailBlock = Mage::getBlockSingleton('vendor/rss_order_details');
  ...
  public function addNewOrderXmlCallback($args) {
    ...
    if ($order && $order->getId()) {
      ...
      $url = Mage::helper('adminhtml')->getUrl('vendor/adminhtml_sales/edit', array('_secure' => true, 'id' => $order->getId(), '_nosecret' => true));
