    	...
    	if(Mage::helper('rewardpoints')->moduleEnabled())
		{
	    	...
	    	if($customer->getId()){
	    		...
	    		if($rewardOrder = Mage::getStoreConfig('rewardpoints/config/reward_point_for_order'))
	    		{
					...
					*/
					//by khanh.phan
					$_point = Mage::helper('rewardpoints/data')->getPointsOnOrder($order, null, $rate);
	    		...
	    		if($product_rewardpoint){
		            ...
		            foreach($quote->getAllVisibleItems() as $item)
		            {
						...
						$_pointsrule =  Mage::getModel('rewardpoints/catalogpointrules')->getCatalogRulePointsGathered($product);
						
		            	if($product->getData('reward_point_product'))
		            	{
							$_points = $product->getData('reward_point_product'); 
						}
						if($_pointsrule > $_points){
							$_points = $_pointsrule;
						}
						if($_points>0){
		           			$historyData = array('type_of_transaction'=>MW_RewardPoints_Model_Type::PURCHASE_PRODUCT, 'amount'=>((int)$_points) * $item->getQty(), 'balance'=>$_customer->getMwRewardPoint(), 'transaction_detail'=>$product->getId()."|".$order->getIncrementId(), 'transaction_time'=>now(), 'status'=>MW_RewardPoints_Model_Status::PENDING);
		           			$_customer->saveTransactionHistory($historyData);
							}
