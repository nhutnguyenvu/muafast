...
class MW_Rewardpoints_Adminhtml_PointrulesController extends Mage_Adminhtml_Controller_action {
    
    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('rewardpoints/pointrules')
                ->_addBreadcrumb(Mage::helper('rewardpoints')->__('Point Rules'), Mage::helper('rewardpoints')->__('Point Rules'));
    ...
    public function indexAction() {
        $this->_initAction()
                ->_addContent($this->getLayout()->createBlock('rewardpoints/adminhtml_pointrules'))
                ->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        $block = $this->getLayout()->createBlock('rewardpoints/adminhtml_pointrules_edit')
            ->setData('action', $this->getUrl('*/rewardpoints_pointrules/save'));
        ...
        $this
            ->_addContent($block)
            ->_addLeft($this->getLayout()->createBlock('rewardpoints/adminhtml_pointrules_edit_tabs'))
            ->renderLayout();
    }
    ...
    public function saveAction()
    {
        if ($this->getRequest()->getPost()) {
            ...
            try {
                ...
                $model->save();
                $cusgroup =  implode(",",$data['customer_group_ids']);
				$websiteid=implode(",",$data['website_ids']);
                $write = Mage :: getSingleton ( 'core/resource' )-> getConnection ( 'core_write' );		
				$readresult = $write -> query("UPDATE ".Mage::getSingleton('core/resource')->getTableName('mw_rewardpoints_pointrules')." 
				SET customer_group_ids ='".$cusgroup."' ,website_ids='".$websiteid."'
				WHERE rule_id =".$model->getId() );
                /*echo '<pre>';
    ...
    public function newConditionHtmlAction()
    {
        ...
        $model = Mage::getModel($type)
            ->setId($id)
            ->setType($type)
            ->setRule(Mage::getModel('rewardpoints/pointrules'))
